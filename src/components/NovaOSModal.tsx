...
export const NovaOSModal = ({ open, onOpenChange, clientes, onSave, isPending, editData }: NovaOSModalProps) => {
  ...
  const [servicos, setServicos] = useState<LineItem[]>([]);
  ...
  // Sync state when editData changes or modal opens
  useEffect(() => {
    if (!open) return;
    setTab("info");
    setPecaError("");
    ...
    if (editData) {
      setForm(mapEditToForm(editData));
      // Fetch servicos and pecas for this OS
      setLoadingItems(true);
      Promise.all([
        supabase.from("servicos_os").select("*").eq("ordem_servico_id", editData.id),
        supabase.from("pecas_os").select("*, estoque_pecas(nome, preco_venda)").eq("ordem_servico_id", editData.id),
      ]).then(([sRes, pRes]) => {
        if (sRes.data) {
          setServicos(sRes.data.map((s: any) => ({
            id: s.id,
            descricao: s.descricao,
            valor: Number(s.valor).toFixed(2).replace(".", ","),
          })));
        }
        if (pRes.data) {
          setPecas(pRes.data.map((p: any) => ({
            ...
            preco_unitario: p.estoque_pecas ? Number(p.estoque_pecas.preco_venda) : (p.quantidade > 0 ? Number(p.valor) / p.quantidade : Number(p.valor)),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <DialogFooter className="mt-4">
          ...
          <Button onClick={handleSubmit} disabled={isPending || !form.cliente_id || loadingItems}>
